...
class ModuleBase:
    ...
    def swipe_and_appear_then_click(self, check_obj, click_obj,
                                    swipe_points_1=([0, 322], [1200, 314]),
        ...
        if left and not right:
            ...
            self.device.swipe(swipe_points_2[0], swipe_points_2[1])
        else:
            # 两个都为False或都为True时默认左滑
            self.device.swipe(swipe_points_1[0], swipe_points_1[1])
        m += 1
        last_swipe_time = time.time()

        time_limit = Timer(swipe_interval, count=1).start()
