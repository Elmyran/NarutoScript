...
from tasks.store_purchase.assets.assets_store_purchase import STORE_ITEM_SEARCH_AREA
...
class MeritExchangeSelector(StoreSelector):
    def search(self, keyword):
        if not StoreItemList.search_rows(main=self.main,keyword=keyword):
            return False
        return True
    def recognition(self,keyword):
        ocr=Ocr(STORE_ITEM_SEARCH_AREA)
